...
router.post('/', async (req, res) => {
  ...
  const orderQuantity = Number.parseInt(quantity, 10);
  const shippingAddress = typeof shipping_address === 'string' ? shipping_address.trim() : '';
  ...
  try {
    ...
    if (inventoryUpdateResult.rows.length === 0) {
      const productExistsResult = await client.query('SELECT id FROM products WHERE id = $1', [productId]);
      await client.query('ROLLBACK');
    ...
    const product = inventoryUpdateResult.rows[0];
    ...
    const orderResult = await client.query(
      ...
      [customerId, productId, orderQuantity, totalAmount, shippingAddress]
    );
