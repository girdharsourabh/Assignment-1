...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    // 1. Get the order and check if it exists & is cancellable, lock it
    const orderResult = await client.query(
    ...
    // 2. Delete the order (or we could update status to 'cancelled', but request asked to cancel from orders table)
    await client.query('DELETE FROM orders WHERE id = $1', [req.params.id]);

    // 3. Restore the inventory
    await client.query(
      'UPDATE products SET inventory_count = inventory_count + $1 WHERE id = $2',
    ...
    await client.query('COMMIT');
    res.json({ message: 'Order cancelled successfully and inventory restored' });
  } catch (err) {
    await client.query('ROLLBACK');
